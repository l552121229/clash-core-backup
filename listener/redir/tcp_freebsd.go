...
func getorigdst(fd uintptr) (netip.AddrPort, error) {
	...
	_, _, err := syscall.Syscall6(syscall.SYS_GETSOCKOPT, fd, syscall.IPPROTO_IP, SO_ORIGINAL_DST,
...
func getorigdst6(fd uintptr) (netip.AddrPort, error) {
	...
	size := uint32(unsafe.Sizeof(addr))
	_, _, err := syscall.Syscall6(syscall.SYS_GETSOCKOPT, fd, syscall.IPPROTO_IPV6, IP6T_SO_ORIGINAL_DST,
		uintptr(unsafe.Pointer(&addr)), uintptr(unsafe.Pointer(&size)), 0)
	if err != 0 {
		return netip.AddrPort{}, err
	}
